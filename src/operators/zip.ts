...
export function zip<T, TResult>(this: IEnumerable<T>, ...params: unknown[]): IEnumerable<TResult> {
  if (params.length < 2) {
    throw new TypeError('zip requires at least one sequence and a result selector.');
  }

  const resultSelector = params[params.length - 1];
  if (typeof resultSelector !== 'function') {
    throw new TypeError('The final zip argument must be a function.');
  }

  const sources = [
    this as EnumerableInput<unknown>,
    ...params.slice(0, -1) as EnumerableInput<unknown>[],
  ];
  ...
  return fromGenerator(function* () {
    ...
    while (true) {
      ...
      yield (resultSelector as (...values: unknown[]) => TResult)(...values, index++);
    }
  });
}
