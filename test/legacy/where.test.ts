...
describe("WhereSelectEnumerable", () => {
  ...
  test("where", function () {
      ...
      deepEqual(seq.where((value, index) => index%2==0).toArray(), [5, 7, 9, 11, 13]);
  });
  ...
  test('applies where after a where-select chain', () => {
      ...
      deepEqual(seq.where(value => value % 2 === 0).select((value, index) => value + index * 2).where((_, index) => index % 2 === 0).toArray(), [6, 14, 22]);
      deepEqual(seq.where((_, index) => index % 2 === 0).select(value => value * 2).where((_, index) => index % 2 === 0).toArray(), [10, 18, 26]);
      deepEqual(seq.where((_, index) => index % 2 === 0).select((value, index) => value + index * 2).where(value => value % 3 === 0).toArray(), [9, 21]);
  });
});
