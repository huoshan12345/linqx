...
describe("Paging", () => {
  test("indexOf", function () {
    ...
    expect(Enumerable.from([1, 10, 100, 1000, 100, 100]).indexOf(100)).toBe(2);

    expect(Enumerable.from([1, 2, 3, 3, 3, 4, 5]).indexOf(3)).toBe(2);
    expect(Enumerable.from([1, 2, 3, 3, 3, 4, 5]).indexOf(function (x) { return x === 3; })).toBe(2);
  });
});
